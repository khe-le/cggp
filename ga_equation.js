...
// Prompt user for desired math formula INFIX
const infixFormula = 'x+y-z';
const prefixFormula = lib.convert_infix_to_prefix(infixFormula).join('');
console.log(prefixFormula)

// Prompt user for number of variables used
const vars = ['x', 'y', 'z'];
const vals = '0123456789' + vars.join('');
let allFitness = [];



// Create a random test case builder - automatically generate random test cases for testing and evaluation

// Also let user define their own test cases
...
genetic.evolve({
  ...
  testCases: [ {x:1, y:5, z:6}, {x: 3, y:7, z:15}, {x:5, y:6, z:1}, {x:9, y:20, z:89}, {x:10, y:40, z:60} ], // Test cases to learn from.
  maxTreeDepth: 25,
  maxLength: 100,
...
// install npm
// install node
// (from command line type:) node ga_equation.js
// variable that contains all fitness: allFitness